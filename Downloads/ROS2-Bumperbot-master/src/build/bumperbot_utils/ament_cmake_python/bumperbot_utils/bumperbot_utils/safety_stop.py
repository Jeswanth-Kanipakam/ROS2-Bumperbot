...
class SafetyStop(Node):
    def __init__(self):
        ...
        # Robot is close to an object -- reduce speed
        self.declare_parameter('warning_distance', 0.6)

        # Robot is too close -- stop immediately
        ...
        # LaserScan Subscriber: Listens to laser scan data (/scan by default)
        self.laser_sub = self.create_subscription(
            LaserScan, self.scan_topic, self.laser_callback, 10
        )

        #Bool Publisher: Publishes a topic like /safety_stop to notify if the robot shouls stop
        self.safety_stop_pub = self.create_publisher(
            Bool, self.safety_stop_topic, 10
        )

        #MarkerArrary Publisher: Publishes visualization markers in Rviz for warning and danger zones
        self.zones_pub = self.create_publisher(
            MarkerArray, 'zones', 10
        )

        #Action clients: Interfaces to speed control actions (joy_turbo_decrease, joy_turbo_increase)
        ...
        #Creates two cylinder markers to visually represent the danger and warning rones in Rviz:
        # Yellow(warning) and Red(danger)
        # Radius based on respective distences
        ...
        #the diameter is twice the warning_distance (default:0.6m), so a 1.2m diameter warning cylinder
        warning_zone.scale.x = self.warning_distance * 2
        warning_zone.scale.y = self.warning_distance * 2
        #colour: Yellow(R=1.0, G=1.0, B=0.0)
        #Alpha(a):0.5 = semi-transparent
    ...
    #This function is triggered every time new scan data is received.
    #It loops through laser ranges to determine the closest object:
    #If within danger_distance, it triggers DANGER state
    #If within warning_distance, it triggers Warning state
    #Else, the robot is in free state.

    # Then it:
    # publishes a stop message if danger is detected.
    # Sends a goal to decrease or increase speed using the action clients.
    # Updates marker transperancy in Rviz based on state.
...
#Summary
#this node is part of a robots safety system that uses LIDAR data to detect nearby obstacles and:
# warns the robot to slow down
# stops the robot if too close to an obstacle
# visualizes the warning/danger zone in Rviz